...
function media3(){
 ...
 const info=document.createElement('div');info.style.flex='1';const title=document.createElement('div');title.className='sl';title.textContent='Media3 // пилот';const desc=document.createElement('div');desc.className='sd';desc.textContent='ТЕНЕВАЯ ПРОВЕРКА · ЗВУК ОСТАЁТСЯ В HTML AUDIO';info.append(title,desc);
 const actions=document.createElement('div');actions.className='n54-media3-actions';
 const validate=document.createElement('button');validate.className='btn';validate.textContent='ПРОВЕРИТЬ';validate.onclick=async()=>{validate.disabled=true;try{const r=await api.validateQueue();desc.textContent=`ПРИНЯТО ${r.accepted||0} · ОТКЛОНЕНО ${r.rejected||0}`;window.toast?.('MEDIA3: ОЧЕРЕДЬ ПРОВЕРЕНА')}catch(e){desc.textContent='ОШИБКА: '+String(e?.message||e)}finally{validate.disabled=false}};
 const pilot=document.createElement('button');pilot.className='btn';const sync=()=>{pilot.textContent=api.enabled?'ПИЛОТ ВКЛ':'ПИЛОТ ВЫКЛ';pilot.setAttribute('aria-pressed',String(api.enabled))};sync();pilot.onclick=()=>{const next=!api.enabled;if(next&&!confirm('Media3 подготовит очередь без запуска звука. Текущий плеер останется активным. Продолжить?'))return;api.setEnabled(next);sync();desc.textContent=next?'ПИЛОТ РАЗРЕШЁН · НАЖМИ «ПОДГОТОВИТЬ»':'ТЕНЕВАЯ ПРОВЕРКА · ЗВУК ОСТАЁТСЯ В HTML AUDIO'};
 const prepare=document.createElement('button');prepare.className='btn';prepare.textContent='ПОДГОТОВИТЬ';prepare.onclick=async()=>{if(!api.enabled)return window.toast?.('СНАЧАЛА ВКЛЮЧИ MEDIA3 PILOT');prepare.disabled=true;try{const s=await api.mirrorQueue({play:false});desc.textContent=`EXOPLAYER ГОТОВИТ ОЧЕРЕДЬ · ${s.mediaId||'БЕЗ ID'}`;window.toast?.('MEDIA3: PLAY=FALSE')}catch(e){desc.textContent='ОШИБКА ПОДГОТОВКИ: '+String(e?.message||e)}finally{prepare.disabled=false}};
 actions.append(validate,pilot,prepare);card.append(info,actions);host.append(card);
}
function labels(){
