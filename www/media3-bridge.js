...
const queueItems=()=>{
  ...
  return S.queue.map(id=>S.byId.get(id)).filter(Boolean).map(track=>({
    id:String(track.id||''),
...
const api={
  ...
  async mirrorQueue({play=false}={}){
    ...
    if(!items.length)throw new Error('Media3 queue is empty');
    const index=Math.max(0,S.queue.indexOf(S.queue[S.qpos]));
    const position=typeof au==='function'?(Number(au().currentTime)||0):0;
    return plugin.setQueue({items,index,position,play:!!play});
  }
};
...
if(plugin){
  plugin.probe().then(info=>{
    ...
    document.dispatchEvent(new CustomEvent('n54:media3-ready',{detail:{...info,enabled:enabled()}}));
